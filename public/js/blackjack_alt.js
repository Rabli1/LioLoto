const blackjackSession = window.blackjackSession || null;
const csrfToken = blackjackSession ? document.querySelector('meta[name="csrf-token"]').content : null;
...
function init() {
    ...
    if (!elements.hit || !elements.stay) {
        console.warn('blackjack_alt.js: interface manquante');
        return;
    }
    ...
    if (elements.placeBet) {
        elements.placeBet.onclick = placeBet;
    }

    elements.hit.onclick = hit;
    elements.stay.onclick = stay;
...
function payWin(multiplier) {
    const balance = getCurrentBalance();
    const payout = Math.round(state.betAmount * (1 + multiplier));
...
function persistBalance(amount) {
    if (!blackjackSession || !csrfToken) return;
    fetch(blackjackSession.endpoints.saveBalance, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
            userId: blackjackSession.userId,
            balance: amount,
            game: 'blackjack',
        }),
